...
type ComponentInteraction<
  T,
  C extends CacheType
> = T extends ComponentType.ActionRow
...
export class Component<
  ...
  C extends CacheType
> {
  private static components = new Collection<
    ComponentType,
    Collection<string, ComponentData<any, any, any>>
  >();
  ...
  public static onComponent(interaction: MessageComponentInteraction) {
    ...
    const find = (
      components: Collection<string, ComponentData<any, any, any>> | undefined,
      type: ComponentType
    ) => {
      ...
      const component = components.find(
        (data) => !!getCustomIdParams(data.customId, customId)
      );
      if (component) {
